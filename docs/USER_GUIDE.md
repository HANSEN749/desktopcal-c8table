...
DesktopCal 是一个 Windows 桌面日历窗口，也可以作为独立网页运行。它把事件数据同步到
c8table，附件文件先保存在本机 IndexedDB；常用视图、日历模式、时间记录看板、右侧抽屉、
网页端和 c8table 表格会围绕同一批事件数据联动。
