...
DesktopCal 是一个贴在 Windows 桌面上的半透明日历。它把事件数据同步到 c8table，附件文件先保存在本机 IndexedDB；日历界面、右侧抽屉和 c8table 表格会围绕同一批事件数据联动。

![日历总览](assets/user-guide/01-overview.png)

## 1. 基本界面

左侧是导航和事件数量，中间是月历，右侧是近期安排和周月报预览。顶部的快速添加只负责把一个标题快速加到今天，适合临时记一条待办。

导航区包含四个入口：

| 入口 | 用途 |
| --- | --- |
| 日历总览 | 查看月历、双击日期新增事件、点击已有事件编辑 |
...
Windows 桌面版不是普通置顶窗口。它会贴到桌面 WorkerW/Progman 层：
...
这和“始终置顶”不同。始终置顶会挡住其他窗口；当前设计是只在桌面可见，正常工作窗口优先。
